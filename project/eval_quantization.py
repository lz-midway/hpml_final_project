...
device, eval_iters = 'cuda', 50 if args.quick else 200
...
def main():
    ckpt = '/workspace/non_binary.pt'
    if not os.path.exists(ckpt): print(f"Missing: {ckpt}"); return
    results = []
    print("\n" + "="*70 + "\n GPU QUANTIZATION COMPARISON (bitsandbytes)\n" + "="*70)

    print("\n[1/3] Full precision (BF16)...")
    results.append(evaluate(load_fp(ckpt), "FP32/BF16"))
    torch.cuda.empty_cache()

    print("\n[2/3] bitsandbytes INT8 (GPU)...")
    m = load_fp(ckpt)
    q8 = quantize_bnb_int8(m)
    results.append(evaluate(q8, "BNB-INT8"))
    del q8; torch.cuda.empty_cache()

    print("\n[3/3] bitsandbytes NF4 (GPU)...")
    m = load_fp(ckpt)
    q4 = quantize_bnb_int4(m)
    results.append(evaluate(q4, "BNB-NF4 (4-bit)"))
    del q4; torch.cuda.empty_cache()

    # QAT if exists
    for bits in [8, 4]:
        qat = f'out/qat_int{bits}/qat_final.pt'
        if os.path.exists(qat):
            print(f"\n[QAT] Loading QAT-INT{bits}...")
            from qat import prepare_qat
            m = Transformer(TransformerConfig())
            prepare_qat(m, bits=bits)
            sd = torch.load(qat, map_location='cpu', weights_only=False)['model']
            sd = {k.replace('module._orig_mod.', '').replace('_orig_mod.', ''): v for k, v in sd.items()}
            m.load_state_dict(sd, strict=False)
            m = m.to(device)
            results.append(evaluate(m, f"QAT-INT{bits}"))
            torch.cuda.empty_cache()
